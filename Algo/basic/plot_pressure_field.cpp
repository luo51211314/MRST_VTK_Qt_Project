...
int main(int argc, char** argv) {
    ...
    // ---------------------------------------------------------
    GridInfo grid;
    string gridFilename = "grid_info.csv";
    ...
    if(getline(gridFile, csvLine)) {
        ...
        if(tokens.size() >= 9) {
            ...
            cerr << "Error: Invalid format in grid_info.csv" << endl; return -1;
        }
    }
    gridFile.close();
    ...
    // ---------------------------------------------------------
    string fieldFilename = "final_field.csv";
    ...
    while(getline(fieldFile, csvLine)) {
        vector<double> tokens = parseCSVLine(csvLine);
        if(tokens.size() < 4) continue; // 至少要有 x,y,z,P

        CellData c;
        c.x = tokens[0]; c.y = tokens[1]; c.z = tokens[2];
        c.P = tokens[3];
        // Sw = tokens[4], Sg = tokens[5] (如果有的话)
    ...
    for(const auto& c : cells) {
        ...
        if (slice_z >= (c.z - grid.dz/2.0) && slice_z <= (c.z + grid.dz/2.0)) {
            // 计算图像坐标
            double x_min = c.x - grid.dx/2.0;
            double y_max = c.y + grid.dy/2.0; // 物理Y最大对应图像Y最小

            Point2f p_tl = worldToImg(x_min, y_max, scale, imgH);
            Point2f p_br = worldToImg(x_min + grid.dx, y_max - grid.dy, scale, imgH);
    ...
    // ---------------------------------------------------------
    string fracFilename = "fracture_geometry.csv";
